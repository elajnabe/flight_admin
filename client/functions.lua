...
FUNC.setMenuServerInfo = function()
    local players = GetNumberOfPlayers()
    SendNUIMessage({
        action = 'serverInfo',
        data = {players = players, uptime = uptime, nextRestart = nextRestart}
    })
end

FUNC.setNoClip = function(bool)
...
FUNC.teleportPlayer = function(coords, updateLastCoords)
    ...
    if not coords and marker then
        ...
        while z < 800.0 do
            Wait(0)
            local found, groundZ = GetGroundZFor_3dCoord(mkr.x, mkr.y, z, false)

            if int == 0 then
                int = GetInteriorAtCoords(mkr.x, mkr.y, z)
            ...
            if found then
                FUNC.setPlayerCoords(vehicle, mkr.x, mkr.y, groundZ)
                break
            end

            FUNC.setPlayerCoords(vehicle, mkr.x, mkr.y, z)
            z += inc
        end
        coords = {mkr.x, mkr.y, z }
    end
...
FUNC.spawnVehicle = function(model, coords)
    ...
    SetVehicleNumberPlateText(vehicle, 'FLIGHT')
