...
local _internal_camera, _internal_isFrozen, _internal_pos, _internal_rot = nil, nil, nil, nil
local _internal_vecX, _internal_vecY, _internal_vecZ = nil, nil, nil

local function GetInitialCameraPosition()
    if _G.CAMERA_SETTINGS.KEEP_POSITION and _internal_pos then return _internal_pos end
    return GetGameplayCamCoord()
end

local function GetInitialCameraRotation()
    if _G.CAMERA_SETTINGS.KEEP_ROTATION and _internal_rot then return _internal_rot end
...
function SetFreecamPosition(pos)    
    ...
    if int ~= 0 then PinInteriorInMemory(int) end
...
function SetFreecamRotation(rot)
    ...
    if _internal_rot == rot then return end
...
local function SetFreecamFov(fov)
    SetCamFov(_internal_camera, Clamp(fov, 0.0, 90.0))
end
...
function IsFreecamActive() return IsCamActive(_internal_camera) end
...
function SetFreecamActive(active)
    ...
    if active then
        if cache.vehicle then TaskLeaveVehicle(ped, cache.vehicle, 16) end
